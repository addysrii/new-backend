...
const socketEvents = require('../utils/socketEvents'); // Add this import
...
const setupSocketIO = async (server) => {
  try {
    ...
    chatNamespace.use(async (socket, next) => {
      // Re-use the main authentication middleware logic
      try {
        const token = socket.handshake.auth.token || 
                    socket.handshake.headers.authorization?.split(' ')[1];
        ...
        logger.error(`Chat namespace authentication error: ${error.message}`);
        return next(new Error('Authentication failed'));
      }
    });
    ...
    notificationNamespace.use(async (socket, next) => {
      ...
      try {
        ...
        logger.error(`Notification namespace authentication error: ${error.message}`);
        return next(new Error('Authentication failed'));
      }
    });
      ...
      try {
        ...
        userChats.forEach(chat => {
          if (chat._id) {
            ...
            socketEvents.joinRoom(userId, chat._id.toString());
          }
        });
        ...
        socket.on('join_chat', async (data) => {
          try {
            ...
            socketEvents.joinRoom(userId, chatId);
        ...
        socket.on('typing', async (data) => {
          try {
            ...
            // Update typing status using socketEvents
            socketEvents.updateTypingStatus(userId, chatId, isTyping);
        ...
        socket.on('disconnect', async () => {
          try {
            ...
            // Unregister socket from socketEvents handler
