    ...
    io.on('connection', async (socket) => {
      ...
      try {
        ...
        userChats.forEach(chat => {
          chat._id && socket.join(`chat:${chat._id.toString()}`);
        });
        ...
        socket.on('update_presence', async (data) => {
          try {
            ...
            userChats.forEach(chat => {
              chat._id && socket.to(`chat:${chat._id.toString()}`).emit('user_presence_update', {
                userId,
                status,
                lastSeen: lastSeen || new Date()
              });
        ...
        socket.on('disconnect', async () => {
          try {
            ...
            userChats.forEach(chat => {
              chat._id && socket.to(`chat:${chat._id.toString()}`).emit('user_offline', {
                userId,
                lastActive: new Date()
              });
            });
